...
export default function Produto2() {
  ...
  const images = [
    ...
    { url: 'https://www.bubbles.com.br/cdn/shop/files/perfume-pet-florato-essential-500ml-2560834.jpg?v=1775239748', alt: 'Perfume Florato Essential' }
  ];
        ...
        <section className="max-w-[1200px] mx-auto px-4 py-6 grid grid-cols-1 md:grid-cols-[55%_45%] gap-8 md:gap-12">
          ...
          <div className="flex flex-col">
            ...
            <div className="border-b border-gray-200 p-6 mb-6">
              ...
              {shippingResult && (
                <div className="mt-4 pt-4 border-t border-gray-50 flex justify-between items-center text-[13px] animate-in fade-in slide-in-from-top-2 duration-500">
                  <div className="flex flex-col">
                    <span className="font-black text-black uppercase text-[10px] tracking-tight">Sedex Express</span>
                    <span className="text-gray-400 font-medium">Entrega em até {shippingResult.days} dias</span>
                  </div>
        ...
        <section className="py-12 px-4 bg-white">
          <div className="max-w-[1200px] mx-auto">
            ...
            <div className="flex gap-4 overflow-x-auto pb-4 no-scrollbar">
              {[
                { name: 'Shampoo Basiq 5L', price: 'R$149,90', icon: '🧴' },
                { name: 'Condicionador Essential 5L', price: 'R$139,90', icon: '💆' },
                { name: 'Colônia Pineapple 500ml', price: 'R$59,90', icon: '🌸' },
                { name: 'Máscara Hidratação 1kg', price: 'R$89,90', icon: '✨' }
              ].map((prod, idx) => (
                <div key={idx} className="min-w-[160px] md:min-w-[200px] bg-[#F9F9F9] rounded-[var(--radius-md)] p-4 border border-transparent hover:border-[var(--pink-light)] transition-all cursor-pointer">
                  <div className="w-full aspect-square bg-white rounded-lg mb-3 flex items-center justify-center text-3xl shadow-sm">
                    {prod.icon}
                  </div>
                  <h3 className="font-bold text-[13px] text-[var(--text-dark)] line-clamp-1 mb-1">{prod.name}</h3>
                  <p className="font-black text-[14px] text-[var(--pink)]">{prod.price}</p>
                </div>
              ))}
            </div>
          </div>
